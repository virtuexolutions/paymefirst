...
@section('page-title','Edit Shipping')
@section('content')
...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('shipping.update',$shipping->id)}}">
        ...
        <div class="form-group">
          <label for="days" class="col-form-label">Days <span class="text-danger">*</span></label>
        <input id="days" type="number" name="days" placeholder="Enter Days"  value="{{$shipping->days}}" class="form-control">
        @error('days')
        <span class="text-danger">{{$message}}</span>
        @enderror
        </div>
